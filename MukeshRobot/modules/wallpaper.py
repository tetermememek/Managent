...
async def wall(_, message: Message):
    ...
    try:
        ...
        text = None
    if not text:
        return await message.reply_text("`ᴘʟᴇᴀꜱᴇ ɢɪᴠᴇ ꜱᴏᴍᴇ Qᴜᴇʀʏ ᴛᴏ ꜱᴇᴀʀᴄʜ.`")
    m = await message.reply_text("`ꜱᴇᴀʀᴄʜɪɴɢ ꜰᴏʀ ᴡᴀʟʟᴘᴀᴘᴇʀꜱ...`")
    try:
        url = requests.get(f"https://api.safone.me/wall?query={text}").json()["results"]
        ran = random.randint(0, 3)
        await message.reply_photo(
            photo=url[ran]["imageUrl"],
            caption=f"❣️ **ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ :** {message.from_user.mention}",
